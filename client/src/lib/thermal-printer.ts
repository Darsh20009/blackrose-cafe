...
export interface PrinterSettings {
  ...
  /** ترميز العربية: utf8 للطابعات الحديثة (Xprinter/Epson) | cp864 لطابعات ProPOS/CP864 القديمة */
  arabicEncoding?: 'utf8' | 'cp864';
...
export interface EscPosReceiptData {
  ...
  /** ترميز العربية — utf8 (افتراضي) أو cp864 لطابعات ProPOS/Xprinter القديمة */
  arabicEncoding?: 'utf8' | 'cp864';
}
...
export function buildEscPosReceipt(data: EscPosReceiptData): Uint8Array {
  ...
  buf.push(...enc('BLACK ROSE CAFE'), 0x0a);
...
export async function buildShiftReportCanvas(opts: ShiftReportOpts): Promise<HTMLCanvasElement> {
  ...
  // ── FOOTER ────────────────────────────────────────────────────────────────
  drawDash();
  drawCenter(`BLACK ROSE SYSTEMS — ${new Date().toLocaleString('ar-SA')}`, Math.round(FS * 0.8), false, '#666');
...
export async function buildGenericReportCanvas(opts: GenericReportOpts): Promise<HTMLCanvasElement> {
  ...
  // ── FOOTER ──
  drawDash();
  drawCenter(opts.footer || `BLACK ROSE SYSTEMS — ${new Date().toLocaleString('ar-SA')}`, Math.round(FS * 0.8), false, '#666');
 ...
 */
/**
 * Cloud Print Queue mode — posts the job to the BLACK ROSE CAFE server.
