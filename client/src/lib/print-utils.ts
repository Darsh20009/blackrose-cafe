...
const _ANDROID_OVERLAY_ID = '__br_android_print_layer';
const _ANDROID_STYLE_ID   = '__br_android_print_style';
...
async function _buildFastCustomerEscPos(
  ...
  return buildReceiptBitmapEscPos({
    ...
    logoDataUrl: '/black-rose-logo-receipt.png',
...
export async function buildReceiptPreviewHtml(data: TaxInvoiceData): Promise<string> {
  ...
  const pmLabel =
    ...
    data.paymentMethod === 'qahwa-card' ? 'بطاقة بلاك روز' :
  ...
  // ── رأس بلاك روز الكلاسيكي (شعار متباعد الحروف + بـلـاك روز + فاصل cafe · كافيه) ──
  const brandHeaderHtml = `
    <div style="text-align:center;padding:6px 0 10px;">
      <div style="font-size:32px;font-weight:700;color:#000;letter-spacing:8px;text-transform:uppercase;line-height:1;">BLACK ROSE</div>
      <div style="font-size:17px;font-weight:700;color:#000;letter-spacing:4px;line-height:1.3;direction:rtl;margin-top:2px;">بـلـاك روز</div>
