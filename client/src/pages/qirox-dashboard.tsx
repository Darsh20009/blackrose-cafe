...
export default function QiroxDashboard() {
  ...
  useEffect(() => {
    ...
    if (tab === "system" && !dbStatus) loadDbStatus();
  }, [tab]);
  ...
  async function loadDbStatus() {
    try {
      const res = await fetch("/api/qirox/db-status", { headers: apiHeaders() });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white" dir="ltr">
      ...
      <main className="max-w-[1600px] mx-auto p-6">
        ...
        {tab === "system" && (health || dbStatus) && <SystemTab data={health} dbStatus={dbStatus} />}
...
function DbHealthCard({ dbStatus }: { dbStatus: DbStatus }) {
  ...
  const barColor = (pct: number) =>
    pct >= 90 ? "#ef4444" : pct >= 80 ? "#f59e0b" : "#2D9B6E";
  ...
  return (
    <div className="bg-[#111] border border-[#1e1e1e] rounded-xl p-6 space-y-5">
      <div className="flex items-center justify-between">
        ...
        {dbStatus.failoverActive && (
          <span className="text-xs bg-[#f59e0b]/10 text-[#f59e0b] border border-[#f59e0b]/30 px-2 py-1 rounded-md">
            Failover Active
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        <div className="bg-[#0a0a0a] rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-[#2D9B6E]">{dbStatus.totalDbs}</p>
          <p className="text-[#666] text-xs mt-0.5">Total DBs</p>
        </div>
        <div className="bg-[#0a0a0a] rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-white">{dbStatus.storageUsagePct}%</p>
          <p className="text-[#666] text-xs mt-0.5">Active DB Usage</p>
        </div>
        <div className="bg-[#0a0a0a] rounded-lg p-3 text-center">
          <p className="text-xl font-bold text-[#3b82f6]">{formatNumber(dbStatus.currentDocCount)}</p>
          <p className="text-[#666] text-xs mt-0.5">Documents</p>
        </div>
      </div>

      <div className="space-y-4">
        {dbStatus.dbs.map((db) => {
          const pct = dbStatus.storageLimitMb > 0
            ? Math.round((db.storageMb / dbStatus.storageLimitMb) * 100)
            : 0;
          const isActive = db.status === "active";

          return (
            <div key={db.label} className="border border-[#1e1e1e] rounded-lg p-4 space-y-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span
                    className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                    style={{ backgroundColor: statusColor[db.status] ?? "#555" }}
                  />
                  <span className="text-sm font-medium text-white">{db.label}</span>
                  {isActive && (
                    <span className="text-[10px] uppercase tracking-wider text-[#2D9B6E] bg-[#2D9B6E]/10 border border-[#2D9B6E]/30 px-1.5 py-0.5 rounded">
                      Active
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  {db.alertSent ? (
                    <span className="text-[10px] uppercase tracking-wider text-[#f59e0b] bg-[#f59e0b]/10 border border-[#f59e0b]/30 px-1.5 py-0.5 rounded">
                      ⚠ Alert Fired
                    </span>
                  ) : (
                    <span className="text-[10px] uppercase tracking-wider text-[#2D9B6E] bg-[#2D9B6E]/10 border border-[#2D9B6E]/30 px-1.5 py-0.5 rounded">
                  ...
                  <span className="text-[#888] text-xs capitalize">{db.status}</span>
                </div>
              </div>
              ...
              {db.status !== "failed" && (
                <>
                  <div className="flex items-center justify-between text-xs text-[#666]">
                    <span>{db.storageMb.toFixed(1)} MB used</span>
                    <span>of {dbStatus.storageLimitMb} MB limit ({pct}%)</span>
                  </div>
                  <div className="h-2 bg-[#1a1a1a] rounded-full overflow-hidden">
                    <div
                      className="h-full rounded-full transition-all duration-500"
                  ...
                  <p className="text-[#555] text-xs">{formatNumber(db.docCount)} documents</p>
                </>
              )}

              {db.status === "failed" && (
                <p className="text-red-400 text-xs">Connection failed</p>
              )}
            </div>
          );
        })}
...
function SystemTab({ data, dbStatus }: { data: SystemHealth | null; dbStatus: DbStatus | null }) {
  return (
    <div className="space-y-8">
      <div>
      ...
      {dbStatus && <DbHealthCard dbStatus={dbStatus} />}
