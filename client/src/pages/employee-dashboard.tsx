...
import { Coffee, LogOut, ShoppingCart, ClipboardList, User, Award, Gift, Sparkles, Download, IdCard, Settings, BarChart3, Table, Lock, Clock, MonitorSmartphone, ChefHat, Wallet, Warehouse, Eye, Bell, CheckCircle, AlertCircle, Calendar, FileText, MapPin, X, Wifi, WifiOff, Utensils, Menu } from "lucide-react";
...
export default function EmployeeDashboard() {
  ...
  // Set SEO metadata
  useEffect(() => {
    document.title = "Employee Dashboard - BLACK ROSE SYSTEMS | Order Management";
    const metaDesc = document.querySelector('meta[name="description"]');
    if (metaDesc) metaDesc.setAttribute('content', 'BLACK ROSE SYSTEMS Employee Dashboard - Track orders, leave requests and notifications');
  }, []);
  ...
  return (
    <div dir={dir} className="flex h-screen bg-gray-50">
      <EmployeeSidebar employee={employee} onLogout={handleLogout} mobileOpen={mobileSidebarOpen} onMobileClose={() => setMobileSidebarOpen(false)} />
      <main className="flex-1 overflow-auto pb-16 sm:pb-0 min-w-0">
        <div className="flex sm:hidden items-center justify-between px-4 py-3 border-b bg-white sticky top-0 z-40">
          <div className="flex items-center gap-2">
            <button onClick={() => setMobileSidebarOpen(true)} className="p-2 rounded-lg hover:bg-gray-100 transition-colors" aria-label="فتح القائمة">
