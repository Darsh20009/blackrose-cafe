...
export default function EmployeeOrders() {
  ...
  useEffect(() => {
    ...
    if (stored) {
      ...
      setEmployee(emp);
      if (emp?.branchId && !['owner', 'admin'].includes(emp.role)) {
  ...
  const filteredOrders = orders.filter((order) => {
    if (selectedBranchId && order.branchId !== selectedBranchId && order.branchId !== 'main' && order.branchId) return false;
    if (statusFilter !== "all") {
                  ...
                  <Card key={orderId} className="hover-elevate overflow-hidden" data-testid={`card-order-${orderId}`}>
                    <CardContent className="p-4 space-y-3">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                        {order.branchId && (
                          <Badge variant="outline" className="text-[10px] bg-primary/5 border-primary/30 text-primary font-semibold">
                            🏪 {branches.find((b: any) => b.id === order.branchId)?.nameAr || order.branchId}
                          </Badge>
                        )}
                        {order.tableNumber && (
