      ...
      >
        <header className="flex flex-col sm:flex-row items-center justify-between px-3 py-2 sm:px-6 sm:py-3 border-b bg-card gap-2 sm:gap-0">
          <div className="flex items-center gap-2 sm:gap-4 w-full sm:w-auto justify-between sm:justify-start">
            <div className="flex items-center gap-2">
              <img src={blackroseLogo} alt="BLACK ROSE CAFE" className="h-9 sm:h-11 w-auto object-contain" />
              <div>
        ...
        <main className="flex-1 flex overflow-hidden">
          ...
          <section className={`${mobilePanelView === 'products' ? 'flex' : 'hidden'} md:flex flex-1 flex-col overflow-hidden`}>
            ...
            {/* ── Foodics-style Top Action Bar ── */}
            <div className={`${mobilePanelView === 'cart' ? 'hidden' : ''} hidden sm:flex items-center gap-1 px-2 py-2 border-b bg-white shrink-0 overflow-x-auto no-scrollbar`}>
              {([
                { icon: <Printer className="w-5 h-5" />,        label: tc('طباعة','Print'),      onClick: () => { if (lastOrder) handlePrintReceipt(); else setShowPrinterSettings(true); }, badge: undefined, danger: false },
                { icon: <ChefHat className="w-5 h-5" />,        label: tc('مطبخ','Kitchen'),     onClick: () => setShowOrdersPanel(true), badge: newOrdersCount || undefined, danger: false },
                { icon: <XCircle className="w-5 h-5" />,        label: tc('إلغاء','Cancel'),     onClick: () => { if(orderItems.length > 0){ setOrderItems([]); setSplitCashAmount(""); setItemDiscounts({}); broadcastToDisplay("order_cancelled", { items:[], subtotal:0, tax:0, total:0 }); } }, badge: undefined, danger: true },
                { icon: <Percent className="w-5 h-5" />,        label: tc('خصم','Discount'),     onClick: () => setShowManualDiscount(true), badge: undefined, danger: false },
                { icon: <MessageSquare className="w-5 h-5" />,  label: tc('ملاحظات','Notes'),    onClick: () => setShowCustomerInfo(v => !v), badge: undefined, danger: false },
                { icon: <Grid3X3 className="w-5 h-5" />,        label: tc('الطاولات','Tables'),  onClick: () => setShowTablesDialog(true), badge: undefined, danger: false },
                { icon: <FolderOpen className="w-5 h-5" />,     label: tc('فتوحة','Open Bills'), onClick: () => setShowOpenBillsDialog(true), badge: openTableOrders.length || undefined, danger: false },
                { icon: <Car className="w-5 h-5" />,            label: tc('سيارات','Cars'),      onClick: () => setShowCarOrdersPanel(true), badge: carPreparationAlerts.length || undefined, danger: false },
                { icon: <PauseCircle className="w-5 h-5" />,    label: tc('تعليق','Hold'),       onClick: () => holdCurrentCartRef.current(), badge: heldCarts.length || undefined, danger: false },
                { icon: <Plus className="w-5 h-5" />,           label: tc('جديد','New'),         onClick: () => newCartTabRef.current(), badge: undefined, danger: false },
                { icon: <RotateCcw className="w-5 h-5" />,      label: tc('استرجاع','Refund'),   onClick: () => setShowRefundDialog(true), badge: undefined, danger: true },
              ] as { icon: JSX.Element; label: string; onClick: () => void; badge?: number; danger: boolean }[]).map((btn, i) => (
                <button
                  key={i}
                  onClick={btn.onClick}
                  className={`relative flex flex-col items-center justify-center gap-1 rounded-lg px-3 py-2 min-w-[60px] transition-all shrink-0
                    ${btn.danger
                ...
                >
                  {btn.icon}
                  <span className="text-[10px] font-bold leading-none whitespace-nowrap">{btn.label}</span>
                  {btn.badge != null && btn.badge > 0 && (
                    <span className="absolute -top-1 -right-1 bg-amber-400 text-black text-[9px] font-black rounded-full min-w-[16px] h-4 flex items-center justify-center px-0.5">
                      {btn.badge}
                    </span>
                  )}
                </button>
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                {usePoints && pointsDiscount > 0 && (
                  <div className="flex justify-between text-[10px] sm:text-sm text-amber-600">
                    <span className="font-bold">{i18n.language === 'ar' ? 'خصم بطاقة بلاك روز' : 'بطاقة بلاك روز'}</span>
              ...
              <div className="space-y-1 text-sm">
                ...
                {usePoints && pointsDiscount > 0 && (
                  <div className="flex justify-between text-sm text-amber-600">
                    <span className="font-bold">{i18n.language === 'ar' ? 'خصم بطاقة بلاك روز' : 'بطاقة بلاك روز Discount'}</span>
