...
export default function AdminReports() {
  ...
  // ── Real trend calculations vs previous period ─────────────────────────────
  const { currentRevenue, prevRevenue, currentOrderCount, prevOrderCount, currentAvg, prevAvg } = useMemo<{
    currentRevenue: number; prevRevenue: number;
    currentOrderCount: number; prevOrderCount: number;
    currentAvg: number; prevAvg: number;
    revenueTrend: number | null; ordersTrend: number | null; avgTrend: number | null;
  }>(() => {
    const now = new Date();
    ...
    if (timePeriod === 'week') {
      ...
      prevEnd = new Date(currentStart); prevEnd.setTime(prevEnd.getTime() - 1);
    ...
    const curOrders = orders.filter((o: any) => inRange(o, currentStart, currentEnd));
    const prvOrders = orders.filter((o: any) => inRange(o, prevStart, prevEnd));

    const cRev = curOrders.reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
    const pRev = prvOrders.reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
    const cCnt = curOrders.length;
    const pCnt = prvOrders.length;
    const cAvg = cCnt > 0 ? cRev / cCnt : 0;
    const pAvg = pCnt > 0 ? pRev / pCnt : 0;

    const pct = (c: number, p: number) =>
      p === 0 ? null : Number(((c - p) / p * 100).toFixed(1));

    return {
      currentRevenue: cRev, prevRevenue: pRev,
      currentOrderCount: cCnt, prevOrderCount: pCnt,
      currentAvg: cAvg, prevAvg: pAvg,
      revenueTrend: pct(cRev, pRev),
      ordersTrend: pct(cCnt, pCnt),
      avgTrend: pct(cAvg, pAvg),
    };
  }, [orders, timePeriod]);

  const pct = (c: number, p: number) =>
    p === 0 ? null : ((c - p) / p * 100).toFixed(1);

  const periodLabel = timePeriod === 'week' ? tc("من الأسبوع الماضي","vs last week")
    : timePeriod === 'month' ? tc("من الشهر الماضي","vs last month")
    : tc("من العام الماضي","vs last year");
        ...
        <CardContent className="pt-5 pb-4">
          ...
          {trendNum !== null ? (
            <div className={`flex items-center gap-1 mt-2 text-xs font-semibold ${isPositive ? 'text-emerald-600' : 'text-red-500'}`}>
              <ArrowUp className={`w-3 h-3 transition-transform ${isPositive ? '' : 'rotate-180'}`} />
              <span>{Math.abs(trendNum)}% {periodLabel}</span>
            </div>
          ) : (
            <p className="text-xs text-muted-foreground mt-2">{tc("لا توجد بيانات مقارنة","No comparison data")}</p>
          )}
        </CardContent>
      </Card>
    );
    ...
    const html = `
      <div style="font-family:'Cairo',Arial,sans-serif;direction:rtl;width:80mm;max-width:80mm;margin:0 auto;padding:10px;color:#000;font-size:11px;">
        ...
        <div style="text-align:center;margin-top:10px;font-size:10px;color:#777;border-top:1px solid #ccc;padding-top:6px;">
          <div>${brand.nameEn} — www.blackrose.com.sa</div>
  ...
  return (
    <div className="p-6 space-y-8 bg-white dark:bg-background min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatBox label={tc("إيرادات الفترة الحالية", "Current Period Revenue")} value={`${currentRevenue.toFixed(0)} ${tc("ر.س","SAR")}`} trend={pct(currentRevenue, prevRevenue)} />
        <StatBox label={tc("طلبات الفترة الحالية", "Current Period Orders")} value={currentOrderCount} trend={pct(currentOrderCount, prevOrderCount)} />
        <StatBox label={tc("متوسط الطلب", "Avg Order Value")} value={`${currentAvg.toFixed(2)} ${tc("ر.س","SAR")}`} trend={pct(currentAvg, prevAvg)} />
