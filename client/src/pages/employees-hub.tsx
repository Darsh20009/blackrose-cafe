...
const TABS: { id: Tab; label: string; Icon: any }[] = [
  { id: "live",        label: "البث المباشر",   Icon: Activity },
  ...
  { id: "leaderboard", label: "الأفضل مبيعاً",   Icon: Trophy },
  { id: "permissions", label: "الصلاحيات",      Icon: Shield },
  { id: "payroll",     label: "كشف الرواتب",    Icon: FileSpreadsheet },
];
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="font-bold text-sm leading-tight">إدارة الموظفين</h1>
                <p className="text-[10px] text-gray-500 leading-tight">المركز الموحد</p>
              </div>
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Stat label="الكل"       value={summary.total || 0}    Icon={Users}      tone="gray" />
        <Stat label="في الوردية"  value={summary.onShift || 0}  Icon={CheckCircle2} tone="green" />
        <Stat label="في استراحة"  value={summary.onBreak || 0}  Icon={Coffee}     tone="gray" />
        <Stat label="متأخر"       value={summary.late || 0}     Icon={Clock}      tone={summary.late > 0 ? "red" : "gray"} />
      </div>

      {/* Employee rows */}
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between">
          <h2 className="font-semibold text-sm">حالة الموظفين الآن</h2>
          <span className="text-xs text-gray-400">يحدّث تلقائياً</span>
        </div>
        <div className="divide-y divide-gray-100">
          {employees.length === 0 && <EmptyRow label="لا يوجد موظفون نشطون" />}
          {employees.map((e: any) => (
            <div key={e.id} className="px-4 py-3 flex items-center gap-3 hover:bg-gray-50" data-testid={`row-employee-${e.id}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">{ROLE_NAMES[e.role] || e.role} · {e.jobTitle}</p>
              </div>
              <StatusPill status={e.status} isLate={e.isLate} lateMinutes={e.lateMinutes} breakType={e.breakType} />
            </div>
          ))}
        </div>
      </div>
            ...
            <div key={v.id} className="px-4 py-3 flex items-center gap-3 hover:bg-gray-50" data-testid={`row-violation-${v.id}`}>
              ...
              {v.penaltyAmount > 0 && (
                <div className="text-left">
                  <p className="text-sm font-bold text-red-600">-{v.penaltyAmount} ر.س</p>
...
function PerformanceTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading && <SkeletonRows />}
      {!isLoading && employees.length === 0 && <EmptyRow label="لا توجد بيانات" />}
          ...
          <div key={e.id} className="bg-white border border-gray-200 rounded-xl p-4" data-testid={`card-perf-${e.id}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-gray-500">{ROLE_NAMES[e.role] || e.role}</p>
                <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
                  <MiniStat label="حضور" value={e.attendance.score} sub={`${e.attendance.presentDays} يوم · ${e.attendance.lateCount} تأخر`} />
                  <MiniStat label="مهام" value={e.tasks.score} sub={`${e.tasks.completionRate}%`} />
                  <MiniStat label="مبيعات" value={e.sales.score} sub={`${e.sales.orderCount} طلب`} />
                </div>
                {e.violations.count > 0 && (
                  <div className="mt-2 text-xs text-red-600 flex items-center gap-1">
                    <AlertTriangle className="h-3 w-3" /> {e.violations.count} مخالفة · -{e.violations.totalPenalty} ر.س
                  </div>
                )}
              </div>
            </div>
...
function LeaderboardTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading && <SkeletonRows />}
      {!isLoading && board.length === 0 && <EmptyRow label="لا توجد مبيعات في هذه الفترة" />}
        ...
        <div className="divide-y divide-gray-100">
          {board.map((e: any, idx: number) => (
            <div key={e.id} className="px-4 py-3 flex items-center gap-3" data-testid={`row-lb-${e.id}`}>
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-sm truncate">{e.fullName}</p>
                <p className="text-xs text-gray-500">{ROLE_NAMES[e.role] || e.role} · {e.orderCount} طلب</p>
              </div>
              <div className="text-left">
                <p className="font-bold text-sm">{e.sales.toLocaleString('ar-SA')} ر.س</p>
                <p className="text-[10px] text-gray-400">متوسط {e.avgOrderValue} ر.س</p>
              </div>
...
function StatusPill({ status, isLate, lateMinutes, breakType }: any) {
  if (status === "on_break") {
    return <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-gray-700 flex items-center gap-1"><Coffee className="h-3 w-3" />استراحة</span>;
  }
  if (status === "on_shift") {
    if (isLate === 1) {
      return <span className="text-xs px-2.5 py-1 rounded-full bg-red-50 text-red-600 flex items-center gap-1"><Clock className="h-3 w-3" />متأخر {lateMinutes}د</span>;
    }
    return <span className="text-xs px-2.5 py-1 rounded-full bg-primary/10 text-primary flex items-center gap-1"><CheckCircle2 className="h-3 w-3" />في الوردية</span>;
  }
  return <span className="text-xs px-2.5 py-1 rounded-full bg-gray-50 text-gray-400">خارج الدوام</span>;
}
