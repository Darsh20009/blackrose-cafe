...
export default function KitchenDisplay() {
  ...
  // Register this screen as "kitchen" with branchId for per-branch broadcast filtering
  useEffect(() => {
    const stored = localStorage.getItem('currentEmployee') || localStorage.getItem('employee');
    const emp = stored ? (() => { try { return JSON.parse(stored); } catch { return null; } })() : null;
    getRealtimeEngine({ clientType: 'kitchen', branchId: emp?.branchId || undefined, employeeId: emp?.id || undefined });
  }, []);
  ...
  return (
    <div className={cn('min-h-screen flex flex-col transition-colors duration-300 dark', rushMode ? 'bg-red-950 text-white' : 'bg-[#0d0d0d] text-white')}>
      ...
      {/* ── HEADER ──────────────────────────────────────────────────────────────── */}
      <header className="sticky top-0 z-50 bg-[#1a1a1a]/95 backdrop-blur border-b border-white/10 shadow-sm">
        ...
        {/* ── STATION BAR ─────────────────────────────────────────────────────── */}
        <div className="flex items-center gap-0.5 px-3 pb-2 overflow-x-auto no-scrollbar bg-[#111]">
        ...
        {/* ── STATUS TABS ─────────────────────────────────────────────────────── */}
        <div className="flex items-center gap-0 border-t border-white/10 overflow-x-auto no-scrollbar bg-[#111]">
          {([
...
function KdsCard({
  ...
  // Border/accent color (Foodics KDS style: colored left border on dark card)
  const accentColor =
    isDelayed || (isRushMode && order.status !== 'completed') ? '#ef4444' :
    isPriority ? '#f97316' :
    order.status === 'ready' ? '#22c55e' :
    order.status === 'in_progress' ? '#eab308' :
    order.status === 'completed' ? '#6b7280' :
    '#22c55e';

  const borderClass =
    isDelayed || (isRushMode && order.status !== 'completed') ? 'border-red-500 shadow-red-500/30 shadow-lg' :
    isPriority ? 'border-orange-400 shadow-orange-400/30 shadow-lg' :
    order.status === 'ready' ? 'border-green-500 shadow-green-500/30 shadow-lg' :
    order.status === 'in_progress' ? 'border-yellow-400 shadow-yellow-400/20 shadow-md' :
    order.status === 'completed' ? 'border-gray-700 opacity-60' :
    'border-green-500/50';

  return (
    <Card
      className={cn('flex flex-col transition-all duration-200 bg-[#1c1c1e] border-0 border-l-4 text-white rounded-xl overflow-hidden', borderClass)}
      style={{ borderLeftColor: accentColor }}
      data-testid={`card-kds-${order.id}`}
    >
      {/* ── Card Header ──────────────────────────────────────────────────────── */}
      <div className={cn('px-3 pt-3 pb-2', isUrgent && order.status !== 'completed' && 'bg-red-500/10')}>
        ...
        {(order.notes || order.customerNotes) && (
          <div className="mt-1 text-[10px] bg-amber-900/30 text-amber-300 rounded px-1.5 py-0.5 border border-amber-700/50">
            💬 {order.notes || order.customerNotes}
          </div>
        )}
      </div>
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isItemDone ? 'bg-green-900/30 opacity-60' : 'bg-white/5 hover:bg-white/10',
