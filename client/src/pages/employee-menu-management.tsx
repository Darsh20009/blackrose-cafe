...
import { Coffee, ArrowRight, ArrowLeft, CheckCircle, XCircle, Plus, Edit2, Trash2, Sparkles, Upload, ImageIcon, X, FlaskConical, AlertTriangle, Library, ChevronUp, ChevronDown, ListOrdered, Download, Loader2 } from "lucide-react";
...
const [editSelectedBranches, setEditSelectedBranches] = useState<BranchAvailability[]>([]);
 ...
 const [exportSelectedCats, setExportSelectedCats] = useState<Set<string>>(new Set());
...
const [aiEditDescription, setAiEditDescription] = useState("");
 ...
 const handleExportImagesZip = async () => {
   ...
   setIsExporting(true);
   try {
     const params = exportSelectedCats.size > 0
       ? `?categories=${[...exportSelectedCats].join(",")}`
       : "";
     const res = await fetch(`/api/menu/export-images-zip${params}`, { credentials: "include" });
     if (!res.ok) {
       const err = await res.json().catch(() => ({}));
       toast({ variant: "destructive", title: "فشل التصدير", description: err.error || "حدث خطأ" });
       return;
     }
     ...
     a.href = url;
     a.download = `menu-images-${new Date().toISOString().slice(0, 10)}.zip`;
     document.body.appendChild(a);
     a.click();
     setTimeout(() => { URL.revokeObjectURL(url); a.remove(); }, 1000);
     toast({ title: "✅ تم التصدير بنجاح!", description: "تم تنزيل الملف المضغوط" });
 ...
 const handleToggleBranch = (branchId: string, checked: boolean) => {
   if (checked) {
     setSelectedBranches(prev => [...prev, { branchId, isAvailable: 1 }]);
   } else {
     setSelectedBranches(prev => prev.filter(b => b.branchId !== branchId));
   }
 };

 const handleUpdateBranchPrice = (branchId: string, price: number) => {
   setSelectedBranches(prev => prev.map(b => b.branchId === branchId ? { ...b, price: price || undefined } : b));
 };

 const handleToggleEditBranch = (branchId: string, checked: boolean) => {
   if (checked) {
     setEditSelectedBranches(prev => [...prev, { branchId, isAvailable: 1 }]);
   } else {
     setEditSelectedBranches(prev => prev.filter(b => b.branchId !== branchId));
   }
 };

 const handleUpdateEditBranchPrice = (branchId: string, price: number) => {
   setEditSelectedBranches(prev => prev.map(b => b.branchId === branchId ? { ...b, price: price || undefined } : b));
 };
 ...
 const formData = new FormData(e.currentTarget);
   ...
   const updates = {
    ...
    reservationPackages: editReservationPackages.filter(p => p.packageName.trim()),
     ...
     branchAvailability: editSelectedBranches.length > 0 ? editSelectedBranches : undefined,
   };
...
setEditImageUrls((item as any).imageUrls || (item.imageUrl ? [item.imageUrl] : []));
 ...
 setEditSelectedBranches((item.branchAvailability as BranchAvailability[]) || []);
...
<div className="space-y-2 border border-amber-200 rounded-lg p-3 bg-amber-50/30">
  <div className="flex items-center justify-between">
    ...
    <button
      type="button"
      onClick={() => { if (addIsReservation) setAddReservationPackages([]); setAddIsReservation(!addIsReservation); }}
...
{canManageMenu && branches.length > 0 && (
   <div>
     <Label className="text-gray-700 font-semibold flex items-center gap-2">🏪 توفر المنتج في الفروع</Label>
     <p className="text-gray-500 text-xs mb-3">اختر الفروع التي سيتوفر فيها هذا المنتج. يمكنك تحديد سعر مختلف لكل فرع. (اتركه فارغاً للتوفر في جميع الفروع بالسعر الرئيسي)</p>
     <div className="space-y-2 max-h-48 overflow-y-auto bg-gray-50 p-3 rounded-lg border border-gray-200">
       {branches.filter((b: any) => b.isActive === 1 || b.isActive === "1").map((branch: any) => {
         ...
         const branchData = selectedBranches.find(sb => sb.branchId === branchId);
         return (
           <div key={branchId} className={`rounded-lg border transition-all ${isSelected ? 'border-primary/40 bg-primary/5' : 'border-gray-200 bg-white'}`}>
             <div className="flex items-center gap-3 p-2">
               ...
               />
               <label htmlFor={`branch-${branchId}`} className="text-gray-700 text-sm font-medium cursor-pointer flex-1">
                 {branch.nameAr}
               </label>
               {isSelected && (
                 <div className="flex items-center gap-1">
                   <span className="text-xs text-gray-500">سعر خاص:</span>
                   <input
                     type="number"
                     min="0"
                     step="0.5"
                     ...
                     onChange={(e) => handleUpdateBranchPrice(branchId, parseFloat(e.target.value))}
                     className="w-24 h-7 text-sm border border-primary/30 rounded px-2 bg-white text-gray-900 focus:border-primary focus:outline-none"
                     data-testid={`input-branch-price-${branchId}`}
                   />
...
{/* Branch Availability & Per-Branch Pricing — Edit */}
{canManageMenu && branches.length > 0 && (
  <div>
    <Label className="text-gray-700 font-semibold flex items-center gap-2">🏪 توفر المنتج في الفروع</Label>
    <p className="text-gray-500 text-xs mb-3">اختر الفروع التي سيتوفر فيها هذا المنتج مع إمكانية تحديد سعر خاص لكل فرع.</p>
    <div className="space-y-2 max-h-48 overflow-y-auto bg-gray-50 p-3 rounded-lg border border-gray-200">
      {branches.filter((b: any) => b.isActive === 1 || b.isActive === "1").map((branch: any) => {
        const branchId = branch.id;
        const isSelected = editSelectedBranches.some(sb => sb.branchId === branchId);
        const branchData = editSelectedBranches.find(sb => sb.branchId === branchId);
        return (
          <div key={branchId} className={`rounded-lg border transition-all ${isSelected ? 'border-primary/40 bg-primary/5' : 'border-gray-200 bg-white'}`}>
            <div className="flex items-center gap-3 p-2">
              <Checkbox
                ...
                checked={isSelected}
                onCheckedChange={(checked) => handleToggleEditBranch(branchId, !!checked)}
                className="border-primary/50"
                data-testid={`checkbox-edit-branch-${branchId}`}
              />
              <label htmlFor={`edit-branch-${branchId}`} className="text-gray-700 text-sm font-medium cursor-pointer flex-1">
                {branch.nameAr}
              </label>
              {isSelected && (
                <div className="flex items-center gap-1">
                  ...
                  <input
                    ...
                    placeholder="كالأصل"
                    value={branchData?.price ?? ''}
                    onChange={(e) => handleUpdateEditBranchPrice(branchId, parseFloat(e.target.value))}
                    className="w-24 h-7 text-sm border border-primary/30 rounded px-2 bg-white text-gray-900 focus:border-primary focus:outline-none"
                    data-testid={`input-edit-branch-price-${branchId}`}
                  />
                  <span className="text-xs text-gray-400">ر.س</span>
                </div>
              )}
...
<div className="space-y-2 border border-amber-200 rounded-lg p-3 bg-amber-50/30">
  <div className="flex items-center justify-between">
    ...
    <button
      type="button"
      onClick={() => { if (editIsReservation) setEditReservationPackages([]); setEditIsReservation(!editIsReservation); }}
...
/>
 ...
 {/* ── Export Images ZIP Dialog ───────────────────────────────────────── */}
 <Dialog open={isExportDialogOpen} onOpenChange={setIsExportDialogOpen}>
   <DialogContent className="bg-white text-gray-900 max-w-md" dir="rtl">
     <DialogHeader>
       <DialogTitle className="flex items-center gap-2 text-gray-900">
         <Download className="w-5 h-5 text-blue-600" />
         تصدير صور المنتجات — ZIP
       </DialogTitle>
     </DialogHeader>
     <div className="space-y-4 py-2">
       <p className="text-sm text-gray-500">
         اختر الأقسام التي تريد تصدير صورها. اتركها فارغة لتصدير كل الأقسام.
       </p>

       {/* All categories toggle */}
       <div className="border rounded-xl p-3 bg-gray-50 space-y-2">
         <div className="flex items-center justify-between mb-1">
           <span className="text-sm font-semibold text-gray-700">الأقسام</span>
           <button
             type="button"
             onClick={() => setExportSelectedCats(new Set())}
             className="text-xs text-blue-600 hover:underline"
           >
             تحديد الكل (تصدير الكل)
           </button>
         </div>
         ...
         {menuCategories.length > 0 ? (
           menuCategories.map((cat) => {
             const checked = exportSelectedCats.has(cat.id);
             const itemCount = coffeeItems.filter(i => i.category === cat.id).length;
             return (
               <label
                 key={cat.id}
               ...
               >
                 <Checkbox
                   checked={checked}
                   onCheckedChange={(v) => {
                     setExportSelectedCats(prev => {
                       const next = new Set(prev);
                       if (v) next.add(cat.id); else next.delete(cat.id);
                       return next;
                     });
                   }}
                   data-testid={`checkbox-export-cat-${cat.id}`}
                 />
                 <span className="flex-1 text-sm font-medium text-gray-800">{cat.nameAr}</span>
                 <span className="text-xs text-gray-400 bg-gray-200 rounded-full px-2 py-0.5">
                   {itemCount} منتج
                 </span>
                 <span className="text-xs text-gray-400 capitalize bg-blue-50 text-blue-600 rounded-full px-2 py-0.5">
                   {cat.department === 'food' ? '🍽️' : '☕'}
                 </span>
               </label>
             );
           })
         ) : (
           /* Fallback: UNIFIED_CATEGORIES */
           UNIFIED_CATEGORIES.map((cat) => {
             ...
             return (
               ...
               >
                 ...
                 <span className="text-xs text-gray-400 bg-gray-200 rounded-full px-2 py-0.5">{itemCount} منتج</span>
               </label>
             );
           })
         )}
       ...
       <div className="bg-blue-50 rounded-xl p-3 text-sm text-blue-700">
         {exportSelectedCats.size === 0 ? (
           <span>سيتم تصدير صور <strong>جميع الأقسام</strong> ({coffeeItems.length} منتج)</span>
         ) : (
           <span>
             سيتم تصدير{" "}
             <strong>
               {coffeeItems.filter(i => exportSelectedCats.has(i.category)).length}
             </strong>{" "}
             منتج من {exportSelectedCats.size} قسم
           </span>
         )}
       </div>
