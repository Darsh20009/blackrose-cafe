...
export function ReceiptInvoice({ order, variant = "button", precomputedTrackingQr, precomputedZatcaQr }: ReceiptInvoiceProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const payload = btoa(`\x01\x10BLACK ROSE CAFE\x02\x0F312718675800003\x03\x14${new Date(order.createdAt).toISOString()}\x04\x08${totalAmount.toFixed(2)}\x05\x08${vat.toFixed(2)}`);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div style={{ padding: "6px 0 10px", marginBottom: "4px" }}>
          <img
            src="/black-rose-logo-receipt.png"
            alt="BLACK ROSE CAFE"
