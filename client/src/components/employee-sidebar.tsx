...
import { LayoutDashboard, ShoppingCart, ClipboardList, Settings, LogOut, User, BarChart3, Warehouse, Wallet, ChefHat, Table, Coffee, Languages, Clock, Truck, Building2, Brain, Tag, Monitor, Bell, BookOpen, Sparkles, CreditCard, X, Menu } from 'lucide-react';
...
export function EmployeeSidebar({ employee, onLogout, mobileOpen = false, onMobileClose }: EmployeeSidebarProps) {
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-border flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img src={blackroseLogoStaff} alt={brand.platformNameEn} className="w-10 h-10 object-contain rounded-lg" />
          <div>
        ...
        <button onClick={onMobileClose} className="lg:hidden p-2 rounded-lg hover:bg-muted" aria-label="إغلاق">
          <X className="w-5 h-5" />
        </button>
      </div>
      <p className="px-4 pt-3 text-xs text-muted-foreground">{t('sidebar.employee_label', { name: employee?.fullName || t('sidebar.loading') })}</p>

      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        {baseItems.map((item) => {
          const Icon = item.icon;
          const fullPath = location + window.location.search;
          const isActive = item.path.includes('?') ? fullPath === item.path : location === item.path && !window.location.search;
          const showBadge = item.isNotifications && unreadCount > 0;
          return (
            <button key={item.path} onClick={() => handleNav(item.path)}
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg transition-all text-sm ${isActive ? 'bg-primary text-primary-foreground shadow-sm' : 'text-foreground hover:bg-primary/10'}`}
              data-testid={`sidebar-link-${item.path.split('/').pop()}`}>
              <div className="relative shrink-0">
                <Icon className="w-5 h-5" />
                {showBadge && <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 px-0.5 rounded-full bg-red-500 text-white text-[9px] font-bold flex items-center justify-center leading-none ring-1 ring-background">{unreadCount > 99 ? '99+' : unreadCount}</span>}
              </div>
              <span className="font-medium flex-1 text-right">{item.label}</span>
              {showBadge && <NotifBadge count={unreadCount} />}
            </button>
          );
        })}
      ...
      <div className="p-4 border-t border-border space-y-2">
        <Button onClick={toggleLanguage} variant="outline" className="w-full justify-start text-sm" data-testid="button-toggle-language">
          <Languages className="w-4 h-4 ml-2" />
          {i18n.language === 'ar' ? 'English' : 'عربي'}
        </Button>
        <Button onClick={onLogout} variant="outline" className="w-full justify-start text-sm hover:bg-destructive/10 hover:text-destructive" data-testid="button-logout-sidebar">
  ...
  return (
    <>
      {/* Desktop */}
      <div className="hidden lg:flex w-64 bg-background border-l border-border flex-col h-screen sticky top-0 shrink-0">
        <SidebarContent />
      </div>

      {/* Mobile overlay */}
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 bg-black/50 z-40 backdrop-blur-sm" onClick={onMobileClose} />
      )}

      {/* Mobile drawer */}
      <div className={`lg:hidden fixed top-0 right-0 h-full w-72 bg-background border-l border-border z-50 flex flex-col transition-transform duration-300 ease-in-out ${mobileOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <SidebarContent />
      </div>
    </>
  );
