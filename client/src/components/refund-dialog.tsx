...
function buildRefundReceiptHtml(params: {
  refundId: string;
  originalOrderNumber: string | number;
  items: RefundItem[];
  refundAmount: number;
  paymentMethod: RefundPayMethod;
  cashAmount?: number;
  cardAmount?: number;
  reason: string;
  employeeName?: string;
  ...
  const {
    refundId, originalOrderNumber, items, refundAmount,
    paymentMethod, cashAmount, cardAmount, reason, employeeName, date,
  } = params;

  const payLabel =
    paymentMethod === "cash" ? "نقدي" :
    paymentMethod === "card" ? "شبكة / بطاقة" :
    `نقدي (${cashAmount?.toFixed(2)}) + شبكة (${cardAmount?.toFixed(2)})`;

  const itemRows = items.map(item => `
    <tr>
      <td style="padding:4px 6px;">${item.nameAr}</td>
      <td style="padding:4px 6px;text-align:center;">${item.quantity}</td>
      <td style="padding:4px 6px;text-align:left;">${item.subtotal.toFixed(2)}</td>
    </tr>
  `).join("");
...
<style>
  * { margin:0; padding:0; box-sizing:border-box; }
  body { font-family: 'IBM Plex Sans Arabic', Tahoma, sans-serif; font-size:12px; width:80mm; padding:8px; }
  .center { text-align:center; }
  .bold { font-weight:bold; }
  .big { font-size:16px; }
  .huge { font-size:22px; color:#b91c1c; }
  .sep { border-top:1px dashed #999; margin:6px 0; }
  table { width:100%; border-collapse:collapse; }
  th { font-size:10px; color:#555; padding:3px 6px; border-bottom:1px solid #ccc; }
  .total-row td { font-weight:bold; font-size:14px; padding:6px; }
  .refund-badge { background:#b91c1c; color:white; border-radius:4px; padding:2px 8px; display:inline-block; font-size:13px; margin:4px 0; }
</style>
</head>
<body>
  <div class="center bold big">استرجاع / REFUND</div>
  <div class="center refund-badge">إشعار دائن - Credit Note</div>
  <div class="sep"></div>
  <div class="center">الطلب الأصلي رقم: <strong>#${originalOrderNumber}</strong></div>
  <div class="center">رقم الاسترجاع: ${refundId.slice(-8).toUpperCase()}</div>
  <div class="center">${date}</div>
  ${employeeName ? `<div class="center">الكاشير: ${employeeName}</div>` : ""}
  <div class="sep"></div>
  <table>
    <thead><tr>
  ...
  <table>
    <tr class="total-row">
      <td>إجمالي المسترجع</td>
      <td style="text-align:left;color:#b91c1c;">${refundAmount.toFixed(2)} ر.س</td>
    </tr>
    <tr>
      <td style="padding:4px;">طريقة الاسترجاع</td>
      <td style="padding:4px;text-align:left;">${payLabel}</td>
    </tr>
    <tr>
      <td style="padding:4px;">السبب</td>
      <td style="padding:4px;text-align:left;">${reason}</td>
    </tr>
  </table>
  <div class="sep"></div>
  <div class="center" style="font-size:10px;color:#666;">شكراً لتعاملكم معنا<br/>تم الاسترجاع بنجاح</div>
</body>
</html>`;
}
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg max-h-[90vh] flex flex-col" dir="rtl">
        ...
        <ScrollArea className="flex-1 min-h-0 pr-1">
                ...
                <div className="border-2 border-green-200 rounded-xl p-4 bg-green-50 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-green-700">{Number(foundOrder.totalAmount || 0).toFixed(2)} ريال</p>
                    </div>
                    <div>
          ...
          {step === 2 && (
            <div className="space-y-3 py-2">
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">{price.toFixed(2)} ريال للوحدة</p>
                      </div>
              ...
              <div className="flex justify-between items-center p-3 bg-red-50 rounded-xl border border-red-200">
                ...
                <span className="text-xl font-black text-red-600">{refundTotal.toFixed(2)} ريال</span>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4 py-2">
              <div className="p-3 bg-muted/50 rounded-xl border text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">إجمالي الاسترجاع</span>
                  <span className="font-black text-red-600 text-base">{refundTotal.toFixed(2)} ريال</span>
                </div>
              ...
              {payMethod === "split" && (
                <div className="grid grid-cols-2 gap-3 p-3 bg-muted/50 rounded-xl border">
                  ...
                  <p className="col-span-2 text-xs text-muted-foreground text-center">
                    المجموع يجب أن يساوي {refundTotal.toFixed(2)} ريال
                  </p>
                </div>
              )}
