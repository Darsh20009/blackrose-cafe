...
async function executeAction(action: any, event: string, data: any, tenantId?: string): Promise<void> {
  ...
  if (type === "webhook") {
    ...
    await fetch(config.url, {
      method: "POST",
      headers: { "Content-Type": "application/json", "User-Agent": "BLACK ROSE-Automation/1.0" },
      body,
      signal: ctrl.signal,
    ...
    if (!config.to) return;
    const subject = interpolate(config.subject || "BLACK ROSE: {{event}}", { event, ...data });
    ...
    const text = interpolate(config.message || "BLACK ROSE: {{event}}", { event, ...data });
      ...
      try {
        ...
        const r = await fetch(hook.url, {
          ...
          headers: {
            ...
            "X-BLACKROSE-Signature": sig,
            "X-BLACKROSE-Event": event,
            "X-BLACKROSE-Delivery": payload.id,
            "User-Agent": "BLACK ROSE-Webhooks/1.0",
          },
          body,
          signal: ctrl.signal,
        });
...
export const INTEGRATION_CATALOG = [
  ...
  { type: "zapier", category: "automation", nameAr: "Zapier", nameEn: "Zapier", icon: "⚡", fields: ["webhookUrl"], description: "ربط BLACK ROSE بـ 5000+ تطبيق عبر Zapier" },
  ...
  // Generic
  { type: "generic_webhook", category: "messaging", nameAr: "Webhook مخصّص", nameEn: "Generic Webhook", icon: "🔗", fields: ["url", "secret"], description: "أرسل أحداث BLACK ROSE لأي نظام خارجي" },
];
