  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p>شكراً لتعاملك معنا | تم إصدار هذه الفاتورة من نظام BLACK ROSE CAFE</p>
...
function generateOrderNotificationSVG(
  ...
  <text x="48" y="56" font-family="Arial,sans-serif" font-size="26" fill="#7FD4A8" font-weight="bold" letter-spacing="2">BLACK ROSE CAFE</text>
  ${orderTypeBadgeEl}
        ...
        setImmediate(async () => {
          try {
            ...
            const emailSent = await sendOrderNotificationEmail(
              customerEmail,
              customerName || 'عميل BLACK ROSE CAFE',
              order.orderNumber,
              "pending",
  ...
  app.post("/api/erp/invoices", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      const invoice = await ErpAccountingService.createInvoice({
        ...
        issuedBy,
        sellerName: req.body.sellerName || "BLACK ROSE CAFE",
    ...
    try {
      ...
      for (const o of orders) {
        ...
        if (method === 'card' || method === 'mada') {
          ...
          debitAccount = '1103';
          debitAccountName = 'بطاقة بلاك روز';
        }
        ...
        journalEntries.push({
          ...
          paymentMethod: method === 'cash' ? 'نقدي' : (method === 'card' || method === 'mada') ? 'شبكة' : 'بطاقة بلاك روز',
...
`;
      ...
      const systemPrompt = `أنت مساعد ذكاء اصطناعي متخصص لإدارة المقاهي والمطاعم، تعمل لصالح نظام BLACK ROSE CAFE.
أنت خبير في:
