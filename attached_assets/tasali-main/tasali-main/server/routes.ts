...
const mailerooUser = process.env.MAILEROO_USER || 'cafe@tasaliqurmash.com.sa';
*/
  ...
  return `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          <p>شكراً لتعاملك معنا | تم إصدار هذه الفاتورة من نظام TASALI QURUMSH</p>
          <p>© 2025 TASALI QURUMSH - جميع الحقوق محفوظة</p>
        </div>
      </div>
    </body>
    </html>
...
function getAppBaseUrl(): string {
  ...
  return `https://www.tasaliqurmash.com.sa`;
}
...
function generateOrderNotificationSVG(
  ...
  <text x="48" y="56" font-family="Arial,sans-serif" font-size="26" fill="#7FD4A8" font-weight="bold" letter-spacing="2">TASALI QURUMSH</text>
  ${orderTypeBadgeEl}
...
export async function registerRoutes(app: Express): Promise<Server> {
    ...
    try {
      ...
      if (!config) {
        config = await BusinessConfigModel.create({
          tenantId,
          tradeNameAr: "تسالي كرومش",
          tradeNameEn: "TASALI QURUMSH",
          activityType: "both",
          isFoodEnabled: true,
  ...
  app.get("/api/tables/:id/qr-code", async (req, res) => {
    try {
      ...
      const PRODUCTION_DOMAIN = "https://www.tasaliqurmash.com.sa";
  ...
  app.get("/api/payment-methods", async (req, res) => {
    try {
      ...
      if (!pg || pg.qahwaCardEnabled !== false) {
        allMethods.push({ id: 'qahwa-card', nameAr: 'بطاقة تسالي', nameEn: 'Tasali Card', details: 'ادفع ببطاقة الولاء', icon: 'fas fa-gift' });
      }
          ...
          try {
            ...
            const intentBody: any = {
              ...
              billing_data: {
                ...
                email: customerEmail || 'guest@tasaliqurmash.com.sa',
              ...
              notification_url: `${process.env.SITE_URL || 'https://www.tasaliqurmash.com.sa'}/api/payments/paymob/webhook`,
              redirection_url: returnUrl
                ? `${returnUrl}${returnUrl.includes('?') ? '&' : '?'}session=${internalSessionId}`
                : `${process.env.SITE_URL || 'https://www.tasaliqurmash.com.sa'}/payment-return-iframe?session=${internalSessionId}&orderRef=${encodeURIComponent(orderId || internalSessionId)}`,
            };
...
(function(){
  ...
  app.get("/api/employees/:id", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      if (!employee || (employee as any).username === 'qirox') {
  ...
  app.post("/api/employees", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      if (bodyData.phone?.trim()) {
        const existingPhone = await EmployeeModel.findOne({ phone: bodyData.phone.trim() }).lean();
        if (existingPhone) {
          return res.status(400).json({ error: "رقم الهاتف مستخدم مسبقاً" });
        }
      }

      // Use storage.createEmployee to get proper password hashing
      const newEmployee = await storage.createEmployee({
        ...bodyData,
        username: bodyData.username.trim(),
        fullName: bodyData.fullName.trim(),
        phone: bodyData.phone?.trim() || `auto-${nanoid(8)}`,
      ...
      // Don't send password back
      const { password: _, ...employeeData } = newEmployee as any;
    ...
    try {
      ...
      if (discountCode.code === 'qahwa-card') {
        if (!customerId) {
          return res.status(400).json({ 
            valid: false,
            error: "يجب تسجيل الدخول لاستخدام خصم بطاقة تسالي"
          });
        }
  ...
  app.get("/api/wallet/apple-pass", requireCustomerAuth, async (req: CustomerAuthRequest, res) => {
    try {
      ...
      const passTypeId = process.env.APPLE_PASS_TYPE_ID  || "pass.tasaliqurmash.com.sa";
  ...
  app.get("/api/attendance/recent-checkins", async (req: any, res) => {
    try {
      ...
      const emps = await EmployeeModel.find({ id: { $in: empIds }, username: { $ne: 'qirox' } }).select('id fullName role jobTitle imageUrl').lean();
  ...
  app.get("/api/attendance/face-employees", async (req: any, res) => {
    try {
      const { EmployeeModel } = await import("@shared/schema");
      const emps = await EmployeeModel.find({ isActive: 1, username: { $ne: 'qirox' }, faceDescriptors: { $exists: true, $not: { $size: 0 } } })
  ...
  app.get("/api/inventory/raw-items", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      let allBranchStocks: any[] = [];
      try {
        const { BranchStockModel } = await import("@shared/schema");
        allBranchStocks = await BranchStockModel.find({}).lean() as any[];
      } catch (_) {
        // BranchStock collection may not exist yet — proceed with empty stocks
      }
      const enriched = items.map(item => {
        const totalStock = allBranchStocks
          .filter((s: any) => s.rawItemId === item.id)
          .reduce((sum: number, s: any) => sum + (s.currentQuantity || 0), 0);
        // Use stored currentStock if no branch stock records exist yet
        return { ...item, currentStock: totalStock || (item as any).currentStock || 0 };
      });
  ...
  app.post("/api/erp/invoices", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      const invoice = await ErpAccountingService.createInvoice({
        ...
        issuedBy,
        sellerName: req.body.sellerName || "TASALI QURUMSH",
        sellerVatNumber: req.body.sellerVatNumber || "",
      });
      res.json({ success: true, invoice: serializeDoc(invoice) });
    } catch (error: any) {
      res.status(400).json({ error: error.message || "Failed to create invoice" });
    ...
    try {
      ...
      for (const o of orders) {
        ...
        journalEntries.push({
          ...
          paymentMethod: method === 'cash' ? 'نقدي' : (method === 'card' || method === 'mada') ? 'شبكة' : 'بطاقة تسالي',
      ...
      res.setHeader('Content-Disposition', `attachment; filename="tasali-accounting-${period}.csv"`);
...
`;
      ...
      const systemPrompt = `أنت مساعد ذكاء اصطناعي متخصص لإدارة المطاعم والمحلات التجارية، تعمل لصالح نظام TASALI QURUMSH.
أنت خبير في:
...
- استخدم الإيموجي لتحسين القراءة`;
      ...
      const aiKey = process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY || process.env.OPENAI_API_KEY;
      if (!aiKey) return res.status(200).json({ response: "مساعد الذكاء الاصطناعي غير مفعّل — يرجى ضبط مفتاح MOONSHOOT_API_KEY.", configured: false });

      const useMoonshot = !!(process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY);
      const aiEndpoint = useMoonshot ? "https://api.moonshot.ai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions";
      const aiModel = useMoonshot ? "moonshot-v1-32k" : "gpt-4o-mini";
      ...
      res.json({ reply, model: useMoonshot ? `moonshot/${aiModel}` : `openai/${aiModel}` });
    } catch (error: any) {
...
لا تضف أي نص خارج الـ JSON.`;
      ...
      const response = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: { "Authorization": `Bearer ${aiKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ model: "gpt-4o-mini", messages: insightsMsgs, max_tokens: 500, temperature: 0.6 }),
      });

      if (!response.ok) return res.json({ insights: [], stats: { todayRevenue, todayOrders: todayOrders.length, weekRevenue, weekOrders: weekOrders.length, growthPct }, error: "فشل الاتصال بـ OpenAI" });
...
- JSON صحيح فقط`;
      ...
      const aiKey = process.env.OPENAI_API_KEY;
      if (!aiKey) return res.status(200).json({ report: null, configured: false, error: "مفتاح OpenAI غير مضبوط" });
      ...
      if (!response.ok) {
        const errText = await response.text();
        console.error("OpenAI smart-report error:", errText);
        return res.status(500).json({ error: "فشل الاتصال بـ OpenAI" });
      }
...
استخدم لغة تذوق احترافية.`,
      ...
      const menuAiKey = process.env.OPENAI_API_KEY;
      if (!menuAiKey) return res.status(200).json({ message: "المساعد غير مفعّل حالياً. يرجى ضبط OPENAI_API_KEY.", configured: false });

      const menuResponse = await fetch("https://api.openai.com/v1/chat/completions", {
        method: "POST",
        headers: { "Authorization": `Bearer ${menuAiKey}`, "Content-Type": "application/json" },
        body: JSON.stringify({ model: "gpt-4o-mini", messages: menuMsgs, max_tokens: 600, temperature: 0.85 }),
      });

      if (!menuResponse.ok) {
        const errText = await menuResponse.text();
        console.error("OpenAI menu error:", errText);
        return res.status(500).json({ error: "فشل في الاتصال بـ OpenAI" });
      }

      const data = await menuResponse.json() as any;
      const content = data.choices?.[0]?.message?.content || "";
      res.json({ result: content, task, model: "openai/gpt-4o-mini" });
    } catch (error: any) {
  ...
  const QIROX_STUDIO_BASE = "https://www.tasaliqurmash.com.sa/api/v1";
  ...
  const qiroxStudioProxy = async (endpoint: string, res: any) => {
    try {
      ...
      if (!response.ok) {
        return res.status(response.status).json({ error: `TASALI Studio API error: ${response.statusText}` });
      }
      const data = await response.json();
      res.json(data);
    } catch (error: any) {
      console.error(`QIROX Studio proxy error (${endpoint}):`, error.message);
      res.status(500).json({ error: "فشل الاتصال بـ TASALI Studio API" });
    }
  };
  ...
  app.get("/api/employees/live-status", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      const tenantId = req.employee?.tenantId || getTenantIdFromRequest(req) || 'demo-tenant';
      const employees = await EmployeeModel.find({ $or: [{ tenantId }, { tenantId: { $exists: false } }], isActive: 1, username: { $ne: 'qirox' } }).lean() as any[];
  ...
  app.get("/api/employees/leaderboard", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      const emps = await EmployeeModel.find({ id: { $in: empIds }, username: { $ne: 'qirox' } }).lean() as any[];
  ...
  // Helper: call OpenAI for natural-language generation
  ...
  app.get("/api/ai/smart-suggestions", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      const [orders, rawItems, employees, apiErrors, audits] = await Promise.all([
        ...
        EmployeeModel.find({ $or: [{ tenantId }, { tenantId: { $exists: false } }], username: { $ne: 'qirox' } }).lean() as any,
...
4. توصيات الشراء: ماذا يجب شراؤه الآن؟`) + `\n\n📦 بيانات المخزون:\n${stockSummary}\n\n🔍 فروقات آخر جرد:\n${stocktakeDiffs || 'لا يوجد جرد حديث'}`;
      ...
      const apiKey = process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY || process.env.OPENAI_API_KEY;
      if (!apiKey) return res.status(500).json({ error: "AI key not configured" });
      const aiEndpoint = ( process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY) ? "https://api.moonshot.ai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions";
      const aiModel = ( process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY) ? "moonshot-v1-32k" : "gpt-4o-mini";
      ...
      if (!response.ok) {
        console.error("[AI inventory-insights error]", JSON.stringify(data));
        return res.status(502).json({ error: "AI service error", detail: data?.error?.message });
      }
      const answer = data.choices?.[0]?.message?.content;
      if (!answer) {
        console.error("[AI inventory-insights empty]", JSON.stringify(data));
        return res.status(502).json({ error: "Empty AI response", detail: JSON.stringify(data) });
      }
      res.json({ answer });
    } catch (e: any) {
      console.error("[AI inventory-insights exception]", e.message);
      res.status(500).json({ error: e.message });
    }
  });
...
استخدم الأرقام والبيانات المقدمة لك. كن عملياً وحاسماً في توصياتك.`;
  ...
  app.get("/api/digital-twin", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      const [branches, allOrders, branchStocks, rawItems, employees] = await Promise.all([
        ...
        BranchStockModel.find(tF).lean(),
        RawItemModel.find(tF).lean(),
        EmployeeModel ? EmployeeModel.find({ ...tF, isActive: true, username: { $ne: 'qirox' } }).select("branchId role").lean().catch(() => []) : Promise.resolve([]),
      ]);
  ...
  app.post("/api/ai/brand-chat", requireAuth, async (req: AuthRequest, res) => {
    try {
      const kimiKey = process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY || process.env.OPENAI_API_KEY;
      if (!kimiKey) return res.status(503).json({ error: "AI not configured" });
      const kimiEndpoint = ( process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY) ? "https://api.moonshot.ai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions";
      const kimiModel = ( process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY) ? "moonshot-v1-8k" : "gpt-4o-mini";
...
أجب دائماً بالعربية بأسلوب ودي ومهني ومختصر. للأسئلة خارج النطاق اعتذر بلطف.`;
      ...
      const aiRes = await fetch(kimiEndpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${kimiKey}` },
        body: JSON.stringify({ model: kimiModel, messages, max_tokens: 800, temperature: 0.7 }),
      });

      if (!aiRes.ok) {
        const errText = await aiRes.text();
        console.error("[AI brand-chat error]", errText);
  ...
  app.post("/api/ai/accounting-audit", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      ...
      if (!kimiKey) return res.status(503).json({ error: "AI not configured" });
      const kimiEndpoint = ( process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY) ? "https://api.moonshot.ai/v1/chat/completions" : "https://api.openai.com/v1/chat/completions";
      const kimiModel = ( process.env.MOONSHOOT_API_KEY || process.env.KIMI_API_KEY) ? "moonshot-v1-32k" : "gpt-4o-mini";
...
6. التوصيات والتصحيحات المقترحة`;
      ...
      if (!aiRes.ok) {
        const errText = await aiRes.text();
        console.error("[AI accounting-audit error]", errText);
        return res.status(502).json({ error: "AI service unavailable" });
      }
  ...
  // ─── AI AUTO JOURNAL ENTRIES — قيود محاسبية تلقائية بالذكاء الاصطناعي ───
  app.post("/api/ai/auto-journal", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      const tenantId = (req as any).tenantId || "demo-tenant";
      const tF = tFilter(tenantId);
      const today = getSaudiStartOfDay();
      ...
      const [orders, expenses, revenues] = await Promise.all([
        OrderModel.find({ ...tF, status: { $in: ["completed","payment_confirmed"] }, createdAt: { $gte: today, $lte: todayEnd } })
          .select("totalAmount costOfGoods paymentMethod items createdAt").lean(),
        ExpenseErpModel ? ExpenseErpModel.find({ ...tF, date: { $gte: today, $lte: todayEnd } }).lean().catch(() => []) : Promise.resolve([]),
        RevenueModel ? RevenueModel.find({ ...tF, createdAt: { $gte: today, $lte: todayEnd } }).lean().catch(() => []) : Promise.resolve([]),
      ]);

      const totalSales = (orders as any[]).reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
      const totalCOGS = (orders as any[]).reduce((s: number, o: any) => s + (o.costOfGoods || 0), 0);
      const cashSales = (orders as any[]).filter((o: any) => o.paymentMethod === "cash").reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
      const cardSales = (orders as any[]).filter((o: any) => o.paymentMethod !== "cash").reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
      const totalExp = (expenses as any[]).reduce((s: number, e: any) => s + (e.amount || e.totalAmount || 0), 0);
      const vatAmount = totalSales * 0.15 / 1.15;
      ...
      const prompt = `أنت محاسب قانوني متخصص في مطاعم وكافيهات السعودية. 
اكتب القيود المحاسبية اليومية بشكل كامل بناءً على البيانات التالية ليوم ${dateStr}:

📊 بيانات اليوم:
- إجمالي المبيعات (شامل VAT): ${totalSales.toFixed(2)} ريال
- مبيعات كاش: ${cashSales.toFixed(2)} ريال
- مبيعات شبكة/بطاقة: ${cardSales.toFixed(2)} ريال  
- تكلفة البضاعة المباعة (COGS): ${totalCOGS.toFixed(2)} ريال
- ضريبة القيمة المضافة 15%: ${vatAmount.toFixed(2)} ريال
- صافي المبيعات (بدون VAT): ${netSales.toFixed(2)} ريال
- إجمالي المصروفات: ${totalExp.toFixed(2)} ريال
- عدد الطلبات: ${(orders as any[]).length}

اكتب القيود بهذا الشكل الواضح:
قيد (1): [اسم القيد]
من حـ/ [اسم الحساب] ← [المبلغ] ريال
إلى حـ/ [اسم الحساب] ← [المبلغ] ريال
---
ثم في النهاية اكتب ملاحظات مهمة إن وجدت.`;

      const aiRes = await fetch(aiEndpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
        body: JSON.stringify({ model: aiModel, messages: [{ role: "user", content: prompt }], max_tokens: 2000, temperature: 0.1 }),
      });
      const aiData: any = await aiRes.json();
      if (!aiRes.ok) {
        console.error("[auto-journal AI error]", JSON.stringify(aiData));
        return res.status(502).json({ error: "AI service error" });
      }
      const journalText = aiData.choices?.[0]?.message?.content || "لم يتمكن من إنشاء القيود.";
      res.json({ journal: journalText, summary: { totalSales, cashSales, cardSales, totalCOGS, vatAmount, netSales, totalExp, ordersCount: (orders as any[]).length, date: dateStr } });
    } catch (e: any) {
      console.error("[auto-journal]", e.message);
      res.status(500).json({ error: e.message });
    }
  });

  // ─── AI DAILY REPORT — تقرير يومي شامل تلقائي ───
  app.post("/api/ai/daily-report", requireAuth, requireManager, async (req: AuthRequest, res) => {
    try {
      const tenantId = (req as any).tenantId || "demo-tenant";
      const tF = tFilter(tenantId);
      const today = getSaudiStartOfDay();
      ...
      const [todayOrders, yesterdayOrders, rawItems, branchStocks, employees, attendance] = await Promise.all([
        OrderModel.find({ ...tF, status: { $in: ["completed","payment_confirmed"] }, createdAt: { $gte: today, $lte: todayEnd } })
          .select("totalAmount costOfGoods paymentMethod items createdAt").lean(),
        OrderModel.find({ ...tF, status: { $in: ["completed","payment_confirmed"] }, createdAt: { $gte: yesterday, $lt: today } })
          .select("totalAmount").lean(),
        RawItemModel.find(tF).lean(),
        BranchStockModel.find(tF).lean(),
        EmployeeModel.find({ ...tF, isActive: true, username: { $ne: 'qirox' } }).select("nameAr role").lean(),
        AttendanceModel ? AttendanceModel.find({ ...tF, date: { $gte: today, $lte: todayEnd } }).lean().catch(() => []) : Promise.resolve([]),
      ]);

      const todayRev = (todayOrders as any[]).reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
      const yesterdayRev = (yesterdayOrders as any[]).reduce((s: number, o: any) => s + (o.totalAmount || 0), 0);
      const growth = yesterdayRev > 0 ? (((todayRev - yesterdayRev) / yesterdayRev) * 100).toFixed(1) : "N/A";

      const productSales: Record<string, { name: string; qty: number; rev: number }> = {};
      for (const o of todayOrders as any[]) {
        for (const item of (o.items || [])) {
          if (!productSales[item.id]) productSales[item.id] = { name: item.nameAr || item.name, qty: 0, rev: 0 };
          productSales[item.id].qty += item.quantity || 1;
          productSales[item.id].rev += (item.price || 0) * (item.quantity || 1);
        }
      }
      const topProducts = Object.values(productSales).sort((a, b) => b.rev - a.rev).slice(0, 5);

      const lowStock = rawItems.filter((r: any) => {
        const qty = (branchStocks as any[]).filter((b: any) => b.rawItemId === r.id).reduce((s: number, b: any) => s + (b.currentQuantity || 0), 0);
        return qty <= (r.minStock || r.reorderPoint || 1);
      }).map((r: any) => r.nameAr);

      const presentCount = (attendance as any[]).filter((a: any) => a.checkIn).length;
      const dateStr = new Date().toLocaleDateString("ar-SA", { timeZone: "Asia/Riyadh" });
      ...
      const prompt = `أنت مستشار أعمال ذكي. اكتب تقرير يومي شامل ومختصر لمدير الكافيه ليوم ${dateStr}.

📊 البيانات:
• المبيعات اليوم: ${todayRev.toFixed(2)} ريال (${(todayOrders as any[]).length} طلب)
• المبيعات أمس: ${yesterdayRev.toFixed(2)} ريال  
• نمو المبيعات: ${growth}%
• أفضل المنتجات: ${topProducts.map(p => `${p.name}(${p.qty}×)`).join("، ")}
• مواد مخزون منخفضة: ${lowStock.length > 0 ? lowStock.join("، ") : "لا يوجد"}
• الموظفون الحاضرون: ${presentCount} من ${(employees as any[]).length}

اكتب تقريراً منظماً يشمل:
1. 📈 ملخص تنفيذي (3 أسطر)
2. 🏆 أبرز الإنجازات
3. ⚠️ التنبيهات والمخاطر
4. ✅ توصيات فورية (3 نقاط عملية)
اجعل الأسلوب مباشراً وعملياً للمدير.`;

      const aiRes = await fetch(aiEndpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json", "Authorization": `Bearer ${apiKey}` },
        body: JSON.stringify({ model: aiModel, messages: [{ role: "user", content: prompt }], max_tokens: 1500, temperature: 0.3 }),
      });
      const aiData: any = await aiRes.json();
      if (!aiRes.ok) {
        console.error("[daily-report AI error]", JSON.stringify(aiData));
        return res.status(502).json({ error: "AI service error" });
      }
      const report = aiData.choices?.[0]?.message?.content || "لم يتمكن من إنشاء التقرير.";
      res.json({
        report,
        stats: { todayRev, yesterdayRev, growth, ordersCount: (todayOrders as any[]).length, topProducts, lowStockCount: lowStock.length, presentCount, totalEmployees: (employees as any[]).length, date: dateStr }
      });
    } catch (e: any) {
