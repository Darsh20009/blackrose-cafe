...
// Stub object file type for metadata operations
export interface ObjectFileStub {
  name: string;
...
export async function setObjectAclPolicy(
  ...
  if (!objectFile._metadata) objectFile._metadata = {};
  objectFile._metadata[ACL_POLICY_METADATA_KEY] = JSON.stringify(aclPolicy);
}

export async function getObjectAclPolicy(
  objectFile: ObjectFileStub,
): Promise<ObjectAclPolicy | null> {
  const aclPolicy = objectFile._metadata?.[ACL_POLICY_METADATA_KEY];
