...
export default function PrinterSettingsPanel() {
  ...
  async function handleConnectBluetooth() {
    ...
    try {
      ...
      // Force image encoding for Bluetooth — BLE thermal printers cannot render Arabic via ESC/POS charset commands
      savePrinterSettings({ mode: 'bluetooth', bluetoothDeviceName: deviceName, arabicEncoding: 'image' });
      setSettings(loadPrinterSettings());
            ...
            <div className="border-2 border-primary/30 rounded-xl p-4 space-y-3 bg-primary/5">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  >
                    <option value="network">{tc("شبكة (LAN/TCP)", "Network (LAN/TCP)")}</option>
                    <option value="relay">{tc("وكيل محلي (Relay)", "Local Relay")}</option>
          ...
          {savedDevice && !isNetworkMode && !isBluetoothMode && !isUsbConnected && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-3 space-y-2 text-xs text-red-800">
              ...
              <p className="font-semibold text-red-700">
                {tc("أو بدلاً عن ذلك: بدّل نوع الطابعة إلى 'بلوتوث' إذا كانت طابعتك تدعمه.", "Or: Switch printer type to 'Bluetooth' if your printer supports it.")}
              </p>
            </div>
          )}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between gap-4">
            <div>
              <Label className="text-sm font-medium">{tc("نوع الطابعة", "Printer Type")}</Label>
              <p className="text-xs text-muted-foreground mt-0.5">
                {settings.mode === 'bluetooth'
                  ? tc("طابعة بلوتوث (BLE) — Xprinter BT، MUNBYN، TA-900UB", "Bluetooth printer (BLE) — Xprinter BT, MUNBYN, TA-900UB")
                  : tc("اتصال USB مباشر — بدون نوافذ طباعة", "Direct USB — no print dialogs")
                }
              </p>
            </div>
            <Select value={settings.mode === 'bluetooth' ? 'bluetooth' : 'webusb'} onValueChange={(v: any) => { updateSetting('mode', v); setRelayStatus(null); setNetworkStatus(null); }}>
              ...
              <SelectContent>
                ...
                <SelectItem value="webusb">
                  <span className="flex items-center gap-1"><Usb className="w-3 h-3" /> {tc("سلكي (USB)", "Wired (USB)")}</span>
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">{tc("الرابط الذي يستخدمه العملاء (مثل https://tasaliqurmash.com.sa). إذا تركته فارغاً، سيستخدم نفس عنوان المتصفح.", "Public URL customers use (e.g. https://tasaliqurmash.com.sa). If empty, uses the current browser URL.")}</p>
            </div>
            <Input
              type="url"
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="image">🖼️ صورة (TAPOS / صينية)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          {settings.arabicEncoding === 'image' && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-2.5 text-xs text-blue-800 space-y-1">
              <p className="font-semibold">🖼️ وضع الصورة — مناسب لطابعات TAPOS والطابعات الصينية</p>
              <p>تُطبع الفاتورة كصورة بدلاً من نص — يحل مشكلة الأحرف المشوّهة مع أي طابعة بغض النظر عن نوع الترميز.</p>
            </div>
          )}
