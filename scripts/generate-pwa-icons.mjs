...
const logoPath = path.join(root, "attached_assets/blackrose-logo.png");
const logoStaffPath = path.join(root, "attached_assets/blackrose-staff-logo.png");
const outDir = path.join(root, "public");

const BG = "#0A0A0A";

async function makeIcon(srcPath, size, outputPath, { maskable = false, bg = BG } = {}) {
  const padding = maskable ? Math.round(size * 0.18) : Math.round(size * 0.10);
...
console.log("\n🌹 Generating BLACK ROSE customer icons...");
...
console.log("\n🌹 Generating BLACK ROSE staff icons...");
...
console.log("\n✅ All BLACK ROSE icons generated successfully!\n");
